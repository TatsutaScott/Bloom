class Palette {
    ...
    static interpolate(a, b, interp) {
        ...
        return `rgba(${c3[0]},${c3[1]},${c3[2]}, 0.05)`;
    }

    paletteInterpolate(num) {
        const index1 = Math.floor(num * (this.colors.length - 1));
        let index2 = index1 + 1 > this.colors.length - 1 ? 0 : index1 + 1;
        const interp = (num * this.colors.length) - Math.floor(num * this.colors.length);
        return Palette.interpolate(this.colors[index1], this.colors[index2], interp)
    }
