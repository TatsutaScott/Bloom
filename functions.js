...
const variationFunctions = [
    ...
    hyperbolic, diamond, ex, julia, bent, waves, fisheye, popcorn, exponential, power, cosine,
    rings, fan, blob, PDJ, fan2, rings2, eyefish, bubble, vCylinder, vPerspective, vNoise, juliaN,
    juliaScope, blur, gaussian, radialBlur, pie, ngon, curl, rectangles, arch, tangent, vSquare,
    blade, twinTrian, cross
];
...
const ngonParams = {
    ...
    'sides': (Math.floor(Math.random() * 8) + 1) * (Math.round(Math.random()) * 2 - 1),
...
const finalVariationFunctions = [
    linear, sinusoidal, spherical, swirl, horseshoe, polar, handkerchief, heart, disc, spiral,
    hyperbolic, diamond, ex, julia, bent, waves, fisheye, popcorn, exponential, power, cosine,
    rings, fan, blob, PDJ, fan2, rings2, eyefish, bubble, vCylinder, vPerspective, juliaN,
    juliaScope, pie, ngon, curl, rectangles, arch, tangent, blade, twinTrian, cross
];
...
function rings(x, y, func) {
    ...
    let re = modn(r + Math.pow(c, 2), 2 * Math.pow(c, 2)) - Math.pow(c, 2) + r * (1 - Math.pow(c, 2));
...
function pie(x, y) {
    ...
    let t2 = rotation + (((2 * Math.PI) / slices) * (t1 + (Math.random() * thickness)));
...
function curl(x, y) {
    ...
    let output = [
        multiplier * ((x * t1) + (y * t2)),
        multiplier * ((y * t1) - (x * t2))
    ];
...
function arch(x, y) {
    ...
    let output = [
        Math.sin(rand),
        Math.pow(Math.sin(rand), 2) / Math.cos(rand)
    ];
